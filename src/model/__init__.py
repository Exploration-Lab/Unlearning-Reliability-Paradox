...
def get_bits_and_bytes_config(model_args):
    with open_dict(model_args):
...
def get_peft_lora_model(model_cfg: DictConfig, model, lora_args=None):
    if lora_args is None:
        return model
    else:
        dict_conf = OmegaConf.to_container(lora_args, resolve=True)
        peft_model = get_peft_model(model, LoraConfig(**dict_conf))
        return peft_model
...
def get_model(model_cfg: DictConfig):
    ...
    with open_dict(model_args):
        ...
        quantization_config = get_bits_and_bytes_config(model_args)
        if model_args.get("lora_args", None) is not None:
            lora_args = model_args.pop("lora_args")
    try:
        ...
        if lora_args is not None:
            logger.info(f"Applying PEFT to the model with lora config: {lora_args}")
            model = get_peft_lora_model(model_cfg, model, lora_args=lora_args)
