...
def load_inseq_model_tokenizer(model_name, model_path, attribution_method="integrated_gradients"):
    def _get_quantization_config():
        bnb_config = BitsAndBytesConfig(
            load_in_4bit=True,
            bnb_4bit_compute_dtype=torch.bfloat16,
            bnb_4bit_quant_type="nf4",
            # Optional: ensure the model stays on the GPU
            llm_int8_threshold=6.0, 
            llm_int8_enable_fp32_cpu_offload=True 
        )
        return bnb_config
    def _load_model_tokenizer (model_name, model_path, bnb_config):
    ...
    model, tokenizer = _load_model_tokenizer(model_name, model_path, bnb_config)
    model = inseq.load_model( model, attribution_method, n_steps=5, internal_batch_size=1)
    return model, tokenizer
...
def get_experiment_name(args):
    ...
    name = f"{model_name}_{args.attribution_method}_{args.prompt_template}_{dataset_name}"
...
if __name__ == "__main__":
    ...
    model, tokenizer = load_inseq_model_tokenizer(args.model_name, args.model_path, attribution_method=args.attribution_method)
